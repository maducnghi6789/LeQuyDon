...
# ==========================================
# 1. CẤU HÌNH LÕI V19 & AI
# ==========================================
VN_TZ = timezone(timedelta(hours=7))

# Dán API Key vào đây để kích hoạt AI
...
def remove_vietnamese_accents(s):
    ...
    patterns = {'[àáạảãâầấậẩẫăằắặẳẵ]': 'a', '[èéẹẻẽêềếệểễ]': 'e', '[ìíịỉĩ]': 'i', 
                '[òóọỏõôồốộổỗơờớợởỡ]': 'o', '[ùúủũưừứựửữ]': 'u', '[ỳýỷỹỵ]': 'y', '[đ]': 'd',
...
# ==========================================
# 2. PHỤC HỒI DB & QUYỀN ADMIN (V19 CORE)
# ==========================================
def init_db():
...
class V19AIEngine:
    ...
    def call_ai(prompt, f_data=None, f_type=None):
        if GEMINI_API_KEY == "AIzaSyDMdmMYUpqnB5wPxcF94Spy6LkNBdkKh2w": return None
        try:
            inp = [prompt]
...
def main():
    ...
    if st.session_state.role == 'student':
        ...
        with t1:
            if st.button("🚀 SINH ĐỀ 40 CÂU THỰC TẾ (GEMINI AI)"):
                p = "Tạo đề Toán 9 thực tế, 40 câu ngẫu nhiên, độ khó cao, JSON: [{'id':1, 'question':'...', 'options':['A','B','C','D'], 'answer':'A', 'explanation':'...'}]"
            ...
            for _, row in exams.iterrows():
                with st.expander(f"📋 {row['title']}"):
                    if st.button("✍️ Làm bài", key=f"ex_{row['id']}"):
                        p = "Đọc đề này và số hóa thành JSON trắc nghiệm để học sinh làm bài trên web kèm giải thích."
    ...
    else: # PHỤC HỒI ADMIN LÕI & CÁC TAB QUẢN TRỊ
        ...
        with tabs[1]:
            ...
            if tit and f and st.button("🚀 PHÁT ĐỀ"):
                ...
                conn.commit(); st.success("🔥 Đã giao đề!")
