...
# ==========================================
# 4. ENGINE TẠO ĐỀ (CÚ PHÁP CHUẨN - CHỐNG LỖI STRING)
# ==========================================
class ExamGenerator:
    def __init__(self):
        self.exam = []
    ...
    def generate_all(self):
        ...
        hd_1 = rf"💡 **Hướng dẫn:** Biểu thức dưới căn >= 0 nên $x \ge {a1}$."
        ...
        hd_2 = r"💡 **Hướng dẫn:** Thay tọa độ vào phương trình hàm số."
        ...
        hd_3 = r"💡 **Hướng dẫn:** Giải bằng phương pháp cộng đại số."
        ...
        hd_4 = r"💡 **Hướng dẫn:** Dùng Tỉ số lượng giác Tan = Đối / Kề."
        self.build_q(rf"Một vật thể có bóng in trên mặt đất dài {bong}m. Tia nắng tạo với mặt đất góc $\alpha$ (như hình vẽ). Chiều cao vật thể được tính bằng:", rf"${bong} \times \tan \alpha$", [rf"${bong} \times \sin \alpha$", rf"${bong} \times \cos \alpha$", rf"${bong} \times \cot \alpha$"], hd_4, draw_tower_shadow(bong))

        # Khởi tạo 36 câu tự động hoàn toàn ngẫu nhiên để chống lặp
        for i in range(5, 41):
            n1 = random.randint(10, 99)
            n2 = random.randint(100, 999)
            # Dùng chuỗi r"" tiêu chuẩn để chống lỗi backslash
            self.build_q(rf"Câu hỏi logic số {i}: Tính tổng của ${n1} + {n2}$?", f"{n1+n2}", [f"{n1+n2+1}", f"{n1+n2-1}", f"{n1+n2+10}"], "Phép cộng cơ bản.")
...
def main():
    ...
    with st.sidebar:
        ...
        role_map = {"core_admin": "👑 Giám Đốc Hệ Thống", "sub_admin": "🛡 Admin Thành Viên", "teacher": "👨‍🏫 Giáo viên", "student": "🎓 Học sinh"}
    ...
    # ==========================
    # GIAO DIỆN HỌC SINH (Giữ nguyên luồng thi 90 phút)
    # ==========================
    if st.session_state.role == 'student':
        ...
        with tab_ai:
            st.title("Luyện Tập Tự Do Cùng AI")
            if 'exam_data' not in st.session_state: st.session_state.exam_data = None
            if 'user_answers' not in st.session_state: st.session_state.user_answers = {}
            if 'is_submitted' not in st.session_state: st.session_state.is_submitted = False

            if st.button("🔄 LÀM ĐỀ MỚI AI", use_container_width=True):
                gen = ExamGenerator()
    ...
    # ==========================
            ...
            if user_to_edit != "-- Chọn --":
                ...
                with st.form("edit_student_form"):
                    ...
                    if st.form_submit_button("💾 Lưu thay đổi", type="primary"):
                        c.execute("UPDATE users SET fullname=?, password=?, class_name=? WHERE username=?", (edit_name, edit_pwd, edit_class, user_to_edit))
                        conn.commit()
                        st.success("Đã cập nhật!")
                        st.rerun()
                
                # Quyền xóa học sinh của Admin
                if st.session_state.role in ['core_admin', 'sub_admin']:
        ...
        if st.session_state.role in ['core_admin', 'sub_admin']:
            with tab_staff:
                ...
                # ==========================================
                # KHU VỰC VÁ LỖI: FORM TẠO ADMIN THÀNH VIÊN
                # ==========================================
                if st.session_state.role == 'core_admin':
                    st.subheader("🛡️ 1. Quản lý Admin Thành viên")
                    
                    # Form Tạo Admin Thành Viên đã được thêm trở lại!
                    with st.form("add_subadmin_form"):
                        st.info("Tạo tài khoản Quản lý cấp Trường/Khối (Sub Admin)")
                        col1, col2 = st.columns(2)
                        sa_user = col1.text_input("Tài khoản Admin Thành viên (viết liền)")
                        sa_pwd = col2.text_input("Mật khẩu")
                        sa_name = st.text_input("Họ và Tên Admin")
                        
                        if st.form_submit_button("Tạo Admin Thành viên", type="primary"):
                            if sa_user and sa_pwd and sa_name:
                                try:
                                    c.execute("INSERT INTO users (username, password, role, fullname) VALUES (?, ?, 'sub_admin', ?)", 
                                              (sa_user.strip(), sa_pwd.strip(), sa_name.strip()))
                                    conn.commit()
                                    st.success(f"✅ Đã tạo thành công Admin Thành viên: {sa_name}")
                                    st.rerun()
                                except: 
                    ...
                    # Quyền Xóa Admin Thành viên
                    sa_to_del = st.selectbox("Chọn Admin cần xóa:", ["-- Chọn --"] + df_sa['Tài khoản'].tolist())
                    if sa_to_del != "-- Chọn --":
                        if st.button("🗑 Xóa vĩnh viễn Admin này", type="secondary"):
        ...
        with tab_system:
            ...
            if st.button("🚀 Giao bài toàn trường", type="primary"):
                if exam_title:
                    ...
                    conn = sqlite3.connect('exam_db.sqlite')
                    c = conn.cursor()
                    c.execute("INSERT INTO mandatory_exams (title, questions_json, start_time, end_time) VALUES (?, ?, ?, ?)", 
